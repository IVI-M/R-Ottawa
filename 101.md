...
- Extend Covid App  to include predictive models for Covid spread
  - Introduction to Machine Learning: basic (using built-in functions)  and advanced (using `caret` , `tensorflow` and `keras`)

- Add interactivity to our Covid App using `shiny`

- Overview of other `tidyverse` packages: `dplyr` and `dtplyr`, `stingr`
